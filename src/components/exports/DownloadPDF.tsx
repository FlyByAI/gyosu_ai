...
const DownloadPDF: React.FC<DownloadPDFProps> = ({ htmlContent, documentTitle }) => {
  const printContent = () => {
...
`;
    ...
    if (myWindow?.document) {
      ...
      myWindow.document.write(`</head><body><h2>${documentTitle || " "}</h2>`);
      myWindow.document.write('<div>');
      myWindow.document.write(htmlContent.replace(/<\/p><br><\/div>/g, "</p></div>"));
      myWindow.document.write('</div>');
  ...
  return (
    ...
    >
      <p className='me-2 hidden md:block'>Print PDF</p>
      <PrintIcon />
    </button>
  )
}
