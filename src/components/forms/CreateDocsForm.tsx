...
const CreateDocxForm: React.FC<CreateDocsFormProps> = ({ document }) => {
    ...
    return (
        <>
            ...
            {/* Button to download DOCX file */}
            {!isLoading && downloadLinks && (
                <button onClick={() => window.open(downloadLinks.docxUrl, '_blank')} className="mt-4 w-full px-4 py-2 bg-green-400 border-2 text-white">
                    Download DOCX
                </button>
            )}
            ...
            {!isLoading && downloadLinks && (
                <button onClick={() => window.open(downloadLinks.pdfUrl, '_blank')} className="mt-4 w-full px-4 py-2 bg-green-400 border-2 text-white">
                    Download PDF
                </button>
            )}
        </>
